...
per (line, step) pair — is what this replaced: five shell functions per line that
...
def step_still(profile, args):
    ...
    if args.real:
        ...
        names, suffix = export_ref_tex.tex_names(profile), "_real"
    else:
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        ...
        description="Run one camera line's stitch steps",
        epilog="steps: " + ", ".join(STEPS))
    parser.add_argument("line", help="one of: " + ", ".join(P.names()))
    ...
    parser.add_argument("--blend-px", type=float, default=None,
                        help="override the line's seam transition width")
    ...
    parser.add_argument("--crop-bottom", default=None, metavar="auto|none|N",
                        help="asset: override the line's bottom crop")
...
def main(argv=None):
    ...
    if args.video_dir is None:
        args.video_dir = P.default_video_dir(profile)
    ...
    if needs_video and args.video_dir is None and args.config is None:
        raise SystemExit("--video-dir is required for: "
                         + ", ".join(sorted(set(needs_video))))
