...
| 世界镜像 | `neg_v`（Y 向下） | `neg_u`+`neg_v`（文件建成转 180°） | 无 | 无（朝向与 `all.fbx` 一致） | 无 | 无（朝向与 `002.fbx` 一致） |
...
| 杂物过滤 | 不需要 | 不需要 | `planes_only` | **不能开**（平面在判据 Y 带外） | 不能开 | 不能开（同理，Y 带更远） |
...
| 世界跨度 | 50m | 50m | 25.00m × 3.00m（每块宽 3.0~5.5m） | 25.05m × 2.75m（每块宽 1.5~3.5m） | 25.00m × 3.00m | 同 overhead（整体上移 11.47m，纯平移） |
...
./scripts/run_align.sh --video 4                      # 另出 off/on 全景视频
...
`solve(标定图, 新图)` 求出的矩阵把**标定图的坐标映到新图的坐标**，再作用到 UV 上。方向
永远是「已标定的那张历史图 → 现场新图」，与两版标定谁更新无关——反向交叉（v2 拉回 0708）
只是把「标定图」换成 v2 那张，方向不变。两向矩阵实测互为逆：复合后残留位移 0.05px、
旋转 0.01°。

### 交叉矩阵

修正只有和它的替代方案比才知道值不值。每个机位有两版标定、两个时期的数据，每种组合一格，
每格用**同一批像素**渲两遍：`off` 是原标定，`on` 是 UV 被修正过的，差别只有 UV。

|  | 数据时期 1 | 数据时期 2 |
| --- | --- | --- |
| **标定 v1** | 同期：地板线 | **交叉**：v1 漂到时期 2 |
| **标定 v2** | **交叉**：v2 拉回时期 1 | 同期：地板线 |

对角线是参照——标定配自己那期数据就是「没有微动」的样子，对齐器应当几乎不动它。
非对角线才是实验，而且**两个方向都跑**：只在时间正向上有效的修正，说明拟合的是数据集
而不是漂移。

| 单元 | 类型 | off → on |
| --- | --- | --- |
| underwater v1 × 202607 | 同期 | 接缝 0.651 → 0.685 |
| underwater v1 × 202608 | **交叉** | 接缝 0.627 → **0.676**（11 好 3 坏）|
| underwater2 v2 × 202607 | **交叉（反向）** | 接缝 0.754 → **0.834**（10 好 1 坏）|
| underwater2 v2 × 202608 | 同期 | 接缝 0.789 → 0.828 |
| water_entry v1 × 0708 | 同期 | 修正量 0.1px（没动就不动）|
| water_entry v1 × 0807 | **交叉** | 真值误差 50.0 → **17.2px** |
| water_entry2 v2 × 0708 | **交叉（反向）** | 真值误差 50.0 → **17.2px** |
| water_entry2 v2 × 0807 | 同期 | 修正量 0.2px |
| gemini v2 × 0807-H | 负对照 | 修正量 0.1px |

两个入水交叉格能被**打分**而不只是被比较，是因为 `water_entry/water_entry_a` 与
`water_entry2/femto` 是同一份几何（顶点坐标逐位相同，只有 UV 不同）在各自那期的图上手工
重标的，互为对方的真值。两向都收敛到 17.2px，是对称的。

真值只在交叉格有意义：对角线上标定自己的 UV 就是那一帧的真值，拿另一期的手工标定去评分
只会量出两版之间那 50px 固有差距，读起来像「50.0 → 50.1 没有改善」，而那里的正确结果是
修正量 ≈ 0。

**gemini 凑不出交叉，也没装作能凑**：它只有一版标定（20260708 目录里那张
`gemini_camera_1_merged.png` 与 0807 那张像素相关 0.9994，是同一份的副本，不是更早的
一次标定）。那一格改作负对照——拿同一天另一场（相机确实没动）的图去配准，对齐器必须
报 ~0。实测 0.01~0.12px。一个不会「保持不动」的漂移修正器比没有更糟。

### 静图与视频各答一半

静图（`stitch_{off,on}.png`）是中值背景，答的是「结构对不对齐」，这也是评分的依据。
`--video 秒数`（裸给即 4 秒）另出两条全景视频，答的是「观众实际看到什么」——泳者穿过
错缝时的跳动、拉伸、瞬间重影，在一张没有泳者的图里根本看不见。默认不出：每版约 90s，
而静图一对只要约 15s。两版走同一批片段、同一套时间对齐，逐帧可比。

产物：`outputs/<line>/align/<key>/`（`stitch_{off,on}{,_grid,_spans,_heat}.png`、
`video_{off,on}.mp4` 或 `overlay_{off,on}.png`，加 `align.json`/`cameras.csv` 记每台
相机的矩阵与是否采纳），汇总 `outputs/align/summary.csv` 与 `index.html`。
**`outputs/<line>/mesh.json` 不动**——它是 `.swasset` 逐字节编译的输入。
...
- `--only` 只跑部分格子时，`summary.csv` 只含跑过的那几格，页面顶部标注「Partial run」。
  混进上一次的行会让不同代码或数据的结果并排却看不出来。
- **已知局限**：每台相机独立对齐而接缝属于两台，均值涨 0.03~0.08，但接缝里仍有
  1~6 条略变差（两侧修正方向不一致）。彻底解决要在接缝残差上做全局联合优化，
...
- 微动对齐是**逐相机独立**的，而接缝属于相邻两台：接缝 NCC 均值涨 0.03~0.08，但仍
  有 1~6 条略微变差。要全部变好需要在接缝残差上联合求解所有相机的变换，尚未实现。
- gemini 只有一版标定，凑不出「标定版本 × 数据日期」的交叉，矩阵里那一格是负对照
  （证明相机没动时对齐器也不动）而不是修正实验。
