...
**跑的是交叉矩阵，不是单向修正**。每个机位两版标定 × 两个时期数据 = 四格：对角线（标定配自己那期）是「没有微动」的参照，对齐器应报 ~0；非对角线是实验，**两个方向都跑**——只在时间正向有效说明拟合的是数据集而不是漂移。CELLS 是数据，加一格就是加一条记录。

- **谁对齐谁**：`solve(标定图, 新图)` 返回的矩阵把**标定图坐标映到新图坐标**，然后作用在 UV 上。所以永远是「已标定的旧图 → 现场新图」这个方向，与哪一版标定更新无关。反向交叉（v2 拉回 0708）只是把「标定图」换成 v2 的那张，方向没变。实测两向矩阵互为逆（复合后位移 0.05px、旋转 0.01°）。
...
- **`--video 秒数` 另出 off/on 两条全景视频**（裸给即 4 秒），默认不出：每版约 90s，静图一对只要约 15s。静图是中值背景，答的是「结构对不对齐」；视频答的是「观众看到什么」——泳者穿过错缝时的跳动/拉伸/重影，在没有泳者的一帧里看不见。两版走同一批片段、同一套时间对齐，逐帧可比。
- **真值只在交叉格有意义**：对角线上标定自己的 UV 就是那一帧的真值，拿另一期的手工标定去评分只会量出两版之间那 50px 固有差距，读起来像「50.0→50.1 没有改善」，实际那里的正确结果是修正量 ≈0（`uv_shift_px` 已经在报）。所以 `truth_err_*` 只在 `cross=1` 的行有值。
- **gemini 凑不出交叉**：只有一版标定（20260708 目录那张与 0807 那张像素相关 0.9994，是副本不是更早的标定）。那一格改作**负对照**——拿同一天另一场（相机确实没动）的图配准，必须报 ~0；实测 0.01~0.12px。一个不会保持不动的漂移修正器比没有更糟。
- 实测收益（`outputs/align/summary.csv`，接缝为零位移 NCC）：

  | 单元 | off → on | 说明 |
  | --- | --- | --- |
  | underwater v1 × 202607 | 0.651 → 0.685 | 同期，地板线 |
  | **underwater v1 × 202608** | **0.627 → 0.676** | 交叉，11 好 3 坏 |
  | **underwater2 v2 × 202607** | **0.754 → 0.834** | 交叉（反向），10 好 1 坏，增益最大 |
  | underwater2 v2 × 202608 | 0.789 → 0.828 | 同期，地板线 |
  | **water_entry v1 × 0807** | 真值误差 50.0 → **17.2px** | 交叉，有真值 |
  | **water_entry2 v2 × 0708** | 真值误差 50.0 → **17.2px** | 交叉（反向），两向对称 |
  | water_entry / water_entry2 同期 | 修正量 0.1 / 0.2px | 没动就不动 |
  | gemini × 0807-H | 修正量 0.1px | 负对照 |

- **已知局限，不是 bug**：每台相机独立对齐而接缝属于两台，均值涨 0.03~0.08 但仍有 1~6 条接缝略变差（两侧修正方向不一致）。彻底解决要在接缝残差上做全局联合优化，本次不做，方向写在 `python/align/__init__.py`。
