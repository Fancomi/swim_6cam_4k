...
class NvdecSource final {
 public:
  NvdecSource(std::shared_ptr<CudaGlContext> context,
              ...
              // meaningful with `loop_sources` off — the clips are then the
              // whole run, so finishing them is success, not a dead lane.
              bool stop_at_eof = false,
              // The content period every lane wraps on. Recorded clips differ in
              // usable length by tens of milliseconds, so wrapping at each
              // file's own end would let the lanes drift apart by that much per
              // pass. Zero means "use each file's natural end".
              std::chrono::milliseconds loop_period =
                  std::chrono::milliseconds{0},
              // Run-wide anchor for media t=0. Lanes start sequentially and open
              // their readers at different speeds; without a shared anchor each
              // lane freezes its own startup lag in as a permanent content
              // offset. nullptr gives this lane its own anchor.
