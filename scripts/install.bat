...
if exist "%ROOT%\build\assets\generated\underwater.swasset" (
  echo   [SKIP] underwater.swasset 已存在
) else (
  if "%MODE%"=="check" (
    echo   [FAIL] 缺 build\assets\generated\underwater.swasset
    exit /b 1
  )
  ...
  "%VPY%" -m python.stitch underwater extract,asset --backend d3d11
  if errorlevel 1 (
    echo   [FAIL] 水下资产生成失败
    exit /b 1
  )
  echo   [OK]   build\assets\generated\underwater.swasset
)

rem 7.3 build swim_realtime
rem     One build tree per backend: python.stitch looks under
