...
REM      水下 16 路 -> python -m python.stitch underwater
...
rem --- underwater 16 lanes: python -m python.stitch drives every step ------
rem     Same module macOS uses via scripts/run_stitch.sh, so the extract /
rem     asset / build / live behaviour cannot drift between platforms.
:run_underwater
...
set "PY_ARGS=underwater extract,asset,build,live --video-dir "!SAMPLE_DIR!""
set "PY_ARGS=!PY_ARGS! --seconds !DURATION! --backend !BACKEND!"
