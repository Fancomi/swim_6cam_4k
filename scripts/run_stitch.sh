#!/usr/bin/env bash
# 相机拼接统一入口（macOS / Linux）—— 六条线共用这一个脚本。
#
#   ./scripts/run_stitch.sh LINE STEPS [选项…]
#
# LINE  哪条相机线（三个机位，各有旧新两版 FBX；换一份重建的 FBX 就是一条新线）：
#   pool / pool2            六路 4K 俯视泳池（两排机位，距离羽化，5001x2101）
#   underwater / underwater2 水下 16 块平面，一条泳道从下往上看（竖直硬缝，6001x656）
#   overhead / overhead2    水上 2 块平面，同一条泳道从上往下看（4251x511）
#
# STEPS 逗号分隔、按给出顺序执行：
#   extract  FBX          -> outputs/<line>/mesh.json
#   tex      相机首帧      -> outputs/<line>/ref_tex/<camera>.png
#   still    静图 + 网格诊断图 + 视野区间图 + 融合热图（overhead* 再加示意图叠加）
...
#   ./scripts/run_stitch.sh overhead2 extract,still       # 重建版，同一批片段
